...
func TestParse(t *testing.T) {
	for _, tc := range []struct {
		...
		{"text on the alert line", dataURL("> [!warning] This tunnel is publicly accessible.\n> [manage](https://tunnel.pizza/cnuss)"),
			Message{SeverityWarning, "text/markdown", "This tunnel is publicly accessible.\n[manage](https://tunnel.pizza/cnuss)"}, false},
		{"alert line with text only", dataURL("> [!note] Quick tunnels expire."),
			Message{SeverityNote, "text/markdown", "Quick tunnels expire."}, false},
		...
		{"CRLF is LF", dataURL("> [!note]\r\n> one\r\n> two"),
			Message{SeverityNote, "text/markdown", "one\ntwo"}, false},
		{"tabs survive", dataURL("> [!note]\n> a\tb"),
			Message{SeverityNote, "text/markdown", "a\tb"}, false},
