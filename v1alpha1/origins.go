...
// can proxy to.
//
// targets is what turns a container reference into something attach can
// serve; the default is the Docker daemon, and a test passes a stub.
//
...
func bindOrigins(ctx context.Context, targets Targets, display []*url.URL, log *slog.Logger) (*bound, error) {
	...
	for _, origin := range display {
		...
		target, err := targets.Open(ctx, origin.Host, log)
		if err != nil {
			_ = b.Close()
			return nil, err
		}
