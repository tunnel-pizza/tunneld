...
// reports when that run reaches a threshold. libtunnel's prober asks the edge
// and emits EventGone when it says no, but emits is all it does — cloudflared
// keeps retrying a reaped tunnel indefinitely and libtunnel leaves it alone —
// so this verdict is what turns a hostname that resolves nowhere into an exit
...
//
// Nothing here about Established. This used to track it too, because Ready
// once fired on the hostname resolving — a moment before the edge had
// registered the route, when the address still answered 530 — and the run
// needed its own view of when the URL was really up. libtunnel's Ready fires
// on Established now, so the question has one owner and it is not this.
